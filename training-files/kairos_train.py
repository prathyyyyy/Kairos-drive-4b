...
try:
    from deepspeed.ops.adam import DeepSpeedCPUAdam as _DeepSpeedCPUAdam  # type: ignore
    _HAS_DS_CPU_ADAM = True
except (ImportError, Exception):
    _DeepSpeedCPUAdam = None  # type: ignore
...
OUTPUT_S3     = os.getenv("OUTPUT_S3", "")  # optional — reports still write locally if unset
CKPT_LOCAL    = Path("/tmp/kairos_ckpt")
CACHE_DIR     = Path("/tmp/kairos_cache")
REPORTS_LOCAL = Path("/tmp/kairos_reports")
MAX_PTS       = 30_000
MAX_PROMPT    = 512
MAX_TARGET    = 512
IMU_TOKENS    = 8
...
missing_lidar_seen  = 0
lidar_fallbacks_used = 0
skipped_bad_rows    = 0
_lidar_fmt_logged: set = set()   # one-time format log per process
...
def _apply_env_cap_overrides(
    max_pts: int, max_prompt: int, max_target: int, rank: int = 0
) -> Tuple[int, int, int]:
    """Optional env overrides for the mode-derived data caps.

    KAIROS_MAX_PTS / KAIROS_MAX_PROMPT / KAIROS_MAX_TARGET, when set, replace
    the mode defaults — e.g. raise smoke_mode's 256-byte target cap to 512 so
    long reasoning_chain+answer targets are not truncated in long smoke-profile
    runs.  Unset (the default) → values pass through unchanged, preserving the
    exact smoke-tested behavior.
    """
    new_pts    = _env_int("KAIROS_MAX_PTS",    max_pts)
    new_prompt = _env_int("KAIROS_MAX_PROMPT", max_prompt)
    new_target = _env_int("KAIROS_MAX_TARGET", max_target)
    if rank == 0 and (new_pts, new_prompt, new_target) != (max_pts, max_prompt, max_target):
        print(
            f"[mode] env cap overrides: max_pts={max_pts}->{new_pts}  "
            f"max_prompt={max_prompt}->{new_prompt}  "
            f"max_target={max_target}->{new_target}",
            flush=True,
        )
    return new_pts, new_prompt, new_target
...
def _args() -> argparse.Namespace:
    ...
    p.add_argument("--lr_schedule", type=str, default="auto",
                   choices=["auto", "smoke", "budget", "wsd", "constant"],
                   help="LR schedule mode. auto=detect from mode/total_steps. "
                        "constant=fixed LR (smoke). budget=WSD+min_lr floor. "
                        "wsd=full cosine to 0 (full training). "
                        "Override also via KAIROS_LR_SCHEDULE env var.")
    p.add_argument("--min_lr_ratio", type=float, default=None,
                   help="Min LR as fraction of base LR. Default: 1.0 constant, "
                        "0.1 budget, 0.0 wsd. Override via KAIROS_MIN_LR_RATIO env var.")
...
def _ds_config(args: argparse.Namespace) -> dict:
    ...
    if _zero_stage == 3:
        ...
        if getattr(args, 'zero3_debug_safe', False):
            # Conservative ZeRO-3 backward settings:
            # - overlap_comm=False: serialise comm+compute to avoid race conditions
            # - contiguous_gradients=True: keep grad buffers contiguous (reduces fragmentation)
            # - small buckets: fewer concurrent alloc/free ops during parameter gather
            zero_cfg["overlap_comm"] = False
            zero_cfg["contiguous_gradients"] = True
            zero_cfg["reduce_bucket_size"] = 5e6
            zero_cfg["stage3_prefetch_bucket_size"] = 5e6
            zero_cfg["stage3_param_persistence_threshold"] = 1e4
    ...
    # KAIROS_ZERO_OFFLOAD_OPTIMIZER / KAIROS_ZERO_OFFLOAD: offload optimizer state to CPU.
    # KAIROS_ZERO_OFFLOAD_OPTIMIZER — fine-grained: only optimizer state, not param shards.
    # KAIROS_ZERO_OFFLOAD           — broader: also implies optimizer offload (but NOT params).
    # Neither activates param offload; use KAIROS_ZERO_OFFLOAD_PARAMS=true for that.
    # Primary fix for Step 1d g6.12xlarge OOM: DeepSpeedCPUAdam keeps fp32 optimizer state
    # (master weights + first/second moments) entirely on CPU — no ~3 GiB GPU spike.
    _offload_opt_only = (
        os.environ.get("KAIROS_ZERO_OFFLOAD_OPTIMIZER", "").lower() in ("1", "true", "yes") or
        os.environ.get("KAIROS_ZERO_OFFLOAD", "").lower() in ("1", "true", "yes")
    )
    if _zero_stage == 3 and _offload_opt_only and not args.zero_offload:
        zero_cfg["offload_optimizer"] = {"device": "cpu", "pin_memory": True}

    # KAIROS_ZERO_OFFLOAD_PARAMS: offload ZeRO-3 param shards to CPU (more aggressive).
    # Default false — param offload hurts forward/backward speed significantly.
    # Use only for extreme GPU memory cases where optimizer offload alone is insufficient.
    _offload_params_only = os.environ.get("KAIROS_ZERO_OFFLOAD_PARAMS", "").lower() in (
        "1", "true", "yes"
    )
    if _zero_stage == 3 and _offload_params_only and not args.zero_offload:
        zero_cfg["offload_param"] = {"device": "cpu", "pin_memory": True}

    # KAIROS_OPTIMIZER_SAFE_MODE: conservative ZeRO-3 settings to reduce optimizer-step
    # memory fragmentation.  Auto-enabled for smoke/ultra_smoke/budget + ZeRO-3 unless
    # explicitly disabled via KAIROS_OPTIMIZER_SAFE_MODE=false.
    # overlap_comm=False serialises grad reduce and param gather → fewer concurrent allocs.
    _optim_safe_raw  = os.environ.get("KAIROS_OPTIMIZER_SAFE_MODE", "").lower()
    _optim_safe_env  = _optim_safe_raw in ("1", "true", "yes")
    _optim_safe_auto = (
        (_is_ultra or _is_smoke) and _zero_stage == 3 and
        _optim_safe_raw not in ("0", "false", "no")
    )
    if _zero_stage == 3 and (_optim_safe_env or _optim_safe_auto) and not getattr(args, "zero3_debug_safe", False):
        zero_cfg["overlap_comm"]         = False
        zero_cfg["contiguous_gradients"] = True
        zero_cfg["reduce_bucket_size"]   = min(float(zero_cfg.get("reduce_bucket_size", 5e7)), 5e7)
        if "stage3_prefetch_bucket_size" in zero_cfg:
            zero_cfg["stage3_prefetch_bucket_size"] = min(
                float(zero_cfg["stage3_prefetch_bucket_size"]), 5e7
            )
...
def _wsd_lambda(
    ...
    if step < warmup:
        ramp = step / max(1, warmup)
        return min_lr_ratio + ramp * (1.0 - min_lr_ratio)
    if step < warmup + stable:
        return 1.0
    decay_steps = total - warmup - stable
    decay_pos   = step - warmup - stable
    raw = 0.5 * (1.0 + math.cos(math.pi * decay_pos / max(1, decay_steps)))
    return max(raw, min_lr_ratio)


def _pick_lr_schedule(args) -> Tuple[str, float]:
    """Select LR schedule mode and min_lr_ratio from env/args.

    Auto-detection priority:
      1. KAIROS_LR_SCHEDULE env var (explicit override)
      2. --lr_schedule CLI arg
      3. Auto from mode flags and total_steps:
           smoke / total_steps <= 20  → constant  (no decay at all)
           budget / total_steps <= 500 → budget   (WSD + min_lr_ratio=0.1 floor)
           otherwise                  → wsd       (original cosine to 0)

    KAIROS_ALLOW_ZERO_LR=true  forces min_lr_ratio=0.0 (opt-out of floor).
    KAIROS_DISABLE_ZERO_LR=true forces min_lr_ratio≥0.1 (opt-in to floor).
    """
    env_mode = os.environ.get("KAIROS_LR_SCHEDULE", "").lower().strip()
    allow_zero  = os.environ.get("KAIROS_ALLOW_ZERO_LR",   "").lower() in ("1","true","yes")
    disable_zero = os.environ.get("KAIROS_DISABLE_ZERO_LR", "").lower() in ("1","true","yes")

    arg_mode = (getattr(args, "lr_schedule", None) or "auto").lower().strip()

    _VALID = {"smoke", "budget", "wsd", "constant"}
    if env_mode in _VALID:
        mode = env_mode
    elif arg_mode in _VALID:
        mode = arg_mode
    else:
        is_smoke  = getattr(args, "ultra_smoke_mode", False) or getattr(args, "smoke_mode", False)
        is_budget = getattr(args, "budget_mode", False)
        total = args.total_steps
        if is_smoke or total <= 20:
            mode = "constant"
        elif is_budget or total <= 500:
            mode = "budget"
        else:
            mode = "wsd"

    env_min = os.environ.get("KAIROS_MIN_LR_RATIO", "").strip()
    arg_min = getattr(args, "min_lr_ratio", None)
    if env_min:
        min_lr: float = float(env_min)
    elif arg_min is not None:
        min_lr = float(arg_min)
    elif mode in ("constant", "smoke"):
        min_lr = 1.0
    elif mode == "budget":
        min_lr = 0.1
    else:
    ...
    if allow_zero:
        min_lr = 0.0
    if disable_zero and min_lr < 0.1:
        min_lr = 0.1

    return mode, min_lr


def _build_lr_lambda(warmup: int, stable: int, total: int, mode: str, min_lr_ratio: float):
    """Return a closure suitable for torch.optim.lr_scheduler.LambdaLR."""
    if mode in ("constant", "smoke"):
        return lambda s: 1.0
    return lambda s: _wsd_lambda(s, warmup, stable, total, min_lr_ratio)


def _get_all_lrs(optimizer) -> List[float]:
    """Current LR for every param group (used for per-step logging and zero-LR guard)."""
    return [float(g["lr"]) for g in optimizer.param_groups]


def _build_adamw_kwargs(foreach: bool) -> dict:
    """Return signature-safe kwargs for torch.optim.AdamW.

    foreach=True in PyTorch AdamW calls _multi_tensor_adamw which allocates a large
    contiguous temporary tensor list proportional to total param count during
    optimizer.step().  On ml.g6.12xlarge Step 1d this caused a ~3 GiB spike after
    forward+backward both passed cleanly (allocated=13.91 GB → +3.04 GiB OOM).
    foreach=False uses the per-tensor loop path: safe, slightly slower, no large temp buffer.
    fused=False avoids a similar large-allocation fused-kernel path (default; override via
    KAIROS_ADAMW_FUSED=true if the runtime supports a safe fused path).
    """
    sig = inspect.signature(AdamW)
    kwargs: dict = {}
    if "foreach" in sig.parameters:
        kwargs["foreach"] = foreach
    if "fused" in sig.parameters:
        _fused_raw = os.environ.get("KAIROS_ADAMW_FUSED", "").lower()
        kwargs["fused"] = _fused_raw in ("1", "true", "yes")   # default False
...
# Cache eviction tuning (active only when KAIROS_CACHE_MAX_GB > 0)
_CACHE_EVICT_CHECK_EVERY = 500    # downloads between size scans (keeps scans off hot path)
_CACHE_EVICT_MIN_AGE_SEC = 600.0  # never evict files newer than this (in-flight safety)
...
class _S3Cache:
    ...
    Disk safety: KAIROS_CACHE_MAX_GB caps the cache directory size (per cache
    instance / per rank dir).  Default 0 = unlimited (legacy behavior).  Long
    runs that stream mostly-unique samples (e.g. 64k samples × ~7 MB) would
    otherwise grow /tmp unboundedly and can fill the disk mid-run.
    """
    ...
    def __init__(self, cache_dir: Path = CACHE_DIR) -> None:
        ...
        try:
            self._max_bytes = int(
                float(os.environ.get("KAIROS_CACHE_MAX_GB", "0") or 0) * 1e9
            )
        except (TypeError, ValueError):
            self._max_bytes = 0
        self._downloads_since_check = 0
    ...
    def _maybe_evict(self, force: bool = False) -> None:
        """Best-effort LRU eviction of old cache files when over the size cap.

        No-op unless KAIROS_CACHE_MAX_GB > 0.  Scans at most once every
        _CACHE_EVICT_CHECK_EVERY downloads; deletes oldest-mtime files down to
        80% of the cap, never touching files younger than
        _CACHE_EVICT_MIN_AGE_SEC (they may be in use by another worker).
        Errors are swallowed — eviction must never break training.
        """
        if self._max_bytes <= 0:
            return
        with self._global_lock:
            self._downloads_since_check += 1
            if not force and self._downloads_since_check < _CACHE_EVICT_CHECK_EVERY:
                return
            self._downloads_since_check = 0
        try:
            now = time.time()
            ...
            for mtime, size, path in entries:
                if total - freed <= target:
                    break
                if now - mtime < _CACHE_EVICT_MIN_AGE_SEC:
                    break  # entries are mtime-sorted; the rest are newer
                try:
                    os.remove(path)
                    freed += size
                except OSError:
                    continue
            if freed > 0 and int(os.environ.get("RANK", "0")) == 0:
                print(
                    f"[cache] evicted {freed/1e9:.2f} GB from {self._dir} "
                    f"(cap={self._max_bytes/1e9:.1f} GB, was {total/1e9:.2f} GB)",
                    flush=True,
                )
        except Exception as exc:
            print(f"[cache][warn] eviction skipped: {exc}", flush=True)
...
def _normalize_lidar_array(
    pts_raw: np.ndarray, *, source_path: str = "", _fmt_hint: str = ""
) -> np.ndarray:
    """Normalize any LiDAR point array to float32 (N, 4) XYZR.

    Handles:
      - structured dtype arrays  (ZOD .npy: x/y/z/timestamp/intensity/diode_index)
      - regular 2D numeric arrays (N, >=4) or (N, 3)
      - flat 1D arrays divisible by 4 or 3

    Returns np.ndarray, dtype=float32, shape=(N, 4) [X, Y, Z, intensity/reflectance].
    Non-finite rows are dropped. Raises ValueError for unrecognised shapes or no-data.
    """
    _rank = int(os.environ.get("RANK", "0"))

    if pts_raw.dtype.fields is not None:
        # ── Structured dtype (e.g. ZOD .npy with named fields) ─────────────
        names = pts_raw.dtype.names
        fmt_key = f"npy_structured:{names}"
        if fmt_key not in _lidar_fmt_logged and _rank == 0:
            print(f"[lidar] format=npy_structured  fields={names}", flush=True)
            _lidar_fmt_logged.add(fmt_key)
        ...
        y_f = _pick(["y", "Y"])
        z_f = _pick(["z", "Z"])
        i_f = _pick(["intensity", "reflectance", "remission", "i"])

        if x_f is None or y_f is None or z_f is None:
            raise ValueError(
                f"Structured LiDAR array missing x/y/z fields. "
                f"Available: {names}  source: {source_path!r}"
            )

        x = pts_raw[x_f].astype(np.float32)
        y = pts_raw[y_f].astype(np.float32)
        z = pts_raw[z_f].astype(np.float32)

        if i_f is not None:
            raw_i = pts_raw[i_f]
            intensity = raw_i.astype(np.float32)
            do_norm = raw_i.dtype == np.uint8 or (
                len(intensity) > 0 and float(intensity.max()) > 1.0
            )
            if do_norm:
                intensity = intensity / 255.0
                if "intensity_normalized" not in _lidar_fmt_logged and _rank == 0:
                    print("[lidar] intensity_normalized=True", flush=True)
                    _lidar_fmt_logged.add("intensity_normalized")
        else:
            intensity = np.zeros(len(pts_raw), dtype=np.float32)

        pts = np.stack([x, y, z, intensity], axis=1)  # (N, 4) float32

    else:
        # ── Numeric array ────────────────────────────────────────────────────
        if pts_raw.ndim == 2 and pts_raw.shape[1] >= 4:
            pts = pts_raw[:, :4].astype(np.float32, copy=False)
            fmt_key = _fmt_hint or "npy_numeric_Nx>=4"
        elif pts_raw.ndim == 2 and pts_raw.shape[1] == 3:
            pts = np.hstack([
                pts_raw.astype(np.float32),
                np.zeros((len(pts_raw), 1), dtype=np.float32),
            ])
            fmt_key = _fmt_hint or "npy_numeric_Nx3"
        elif pts_raw.ndim == 1:
            sz = pts_raw.size
            if sz % 4 == 0:
                pts = pts_raw.astype(np.float32).reshape(-1, 4)
                fmt_key = _fmt_hint or "npy_numeric_flat4"
            elif sz % 3 == 0:
                p3 = pts_raw.astype(np.float32).reshape(-1, 3)
                pts = np.hstack([p3, np.zeros((len(p3), 1), dtype=np.float32)])
                fmt_key = _fmt_hint or "npy_numeric_flat3"
            else:
                raise ValueError(
                    f"Flat LiDAR array size {sz} not divisible by 3 or 4. "
                    f"source: {source_path!r}"
                )
        else:
            raise ValueError(
                f"Unsupported LiDAR array shape {pts_raw.shape}. "
                f"source: {source_path!r}"
            )

        if fmt_key not in _lidar_fmt_logged and _rank == 0:
            print(f"[lidar] format={fmt_key}", flush=True)
            _lidar_fmt_logged.add(fmt_key)
    ...
    if len(pts) == 0:
        raise ValueError(
            f"LiDAR array has no finite points after filtering "
            f"(total={n_total}).  source: {source_path!r}"
        )
    return pts


def _load_lidar(s3_uri: str, cache: _S3Cache, max_pts: int = MAX_PTS) -> np.ndarray:
    """Load velodyne .bin or ZOD .npy → (max_pts, 4) float32 XYZR."""
    local = str(cache.local(s3_uri))
    if s3_uri.endswith(".npy"):
        pts_raw = np.load(local)
        pts = _normalize_lidar_array(pts_raw, source_path=s3_uri)
    else:
        pts_raw = np.fromfile(local, dtype=np.float32).reshape(-1, 4)
        pts = _normalize_lidar_array(pts_raw, source_path=s3_uri,
                                     _fmt_hint="kitti_bin_numeric")
...
_MISSING_LIDAR_WARN_INTERVAL = 1_000


def _inc_missing_lidar(
    idx: int,
    lidar_t_missing: bool,
    lidar_t1_missing: bool,
) -> None:
    """Increment missing-lidar counters and emit a rate-limited warning."""
    global missing_lidar_seen, lidar_fallbacks_used
    n_fallbacks = int(lidar_t_missing) + int(lidar_t1_missing)
    if n_fallbacks == 0:
        return
    missing_lidar_seen   += 1
    lidar_fallbacks_used += n_fallbacks
    if (missing_lidar_seen == 1
            or missing_lidar_seen % _MISSING_LIDAR_WARN_INTERVAL == 0):
        print(
            f"[WARN][missing_lidar] idx={idx} "
            f"lidar_t_missing={lidar_t_missing} "
            f"lidar_t1_missing={lidar_t1_missing} "
            f"— using zero LiDAR fallback "
            f"(rows with missing lidar: {missing_lidar_seen})",
            flush=True,
        )
...
# Lowercase sentinel strings that represent an absent/unknown path value.
_MISSING_PATH_SENTINELS: frozenset = frozenset(
    {"", "none", "nan", "null", "__missing__"}
)


def _is_missing_path_value(x: object) -> bool:
    """Return True if x represents an absent or placeholder path.

    Covers: None, NaN (float or pd.NA), empty string, whitespace-only,
    and common sentinel strings ("None", "nan", "null", "__missing__").
    """
    if x is None:
        return True
    try:
        ...
        pass
    s = str(x).strip()
    return not s or s.lower() in _MISSING_PATH_SENTINELS
...
def _is_skippable_data_error(exc: BaseException) -> bool:
    """True only for data-loading failures that skip_bad_rows may step over.

    Skippable: missing local files (FileNotFoundError), S3 404/NoSuchKey
    ClientError, and decode/read errors (ValueError, OSError — PIL decode
    errors subclass OSError).  Everything else — model/forward errors,
    non-404 S3 errors (throttling, AccessDenied), schema KeyErrors —
    always propagates regardless of KAIROS_SKIP_BAD_ROWS.
    """
    if isinstance(exc, _CLIENT_ERROR_TYPES):
        return _is_missing_s3_error(exc)
    return isinstance(exc, (FileNotFoundError, ValueError, OSError))


def _inc_skipped_bad_row(idx: int, exc: BaseException) -> None:
    global skipped_bad_rows
    skipped_bad_rows += 1
    reason = f"{type(exc).__name__}: {exc}"
    if len(reason) > 300:
        reason = reason[:300] + "..."
    print(
        f"[data][skip_bad_row] idx={idx} reason={reason} "
        f"(total skipped: {skipped_bad_rows})",
        flush=True,
    )
...
class KairosDataset(Dataset):
    ...
    def __init__(
        ...
        max_prompt: int = MAX_PROMPT,
        max_target: int = MAX_TARGET,
        skip_bad_rows: Optional[bool] = None,
        allow_oxts_fallback: bool = False,
        allow_missing_lidar: Optional[bool] = None,
        max_bad_row_attempts: Optional[int] = None,
    ) -> None:
        ...
        # KAIROS_SKIP_BAD_ROWS=true lets long runs step over rows whose S3
        # assets are missing/corrupt instead of crashing.  Default strict.
        if skip_bad_rows is None:
            skip_bad_rows = _env_flag("KAIROS_SKIP_BAD_ROWS", False)
        self._skip_bad_rows = skip_bad_rows
        if max_bad_row_attempts is None:
            max_bad_row_attempts = _env_int("KAIROS_MAX_BAD_ROW_ATTEMPTS", 100)
        self._max_bad_row_attempts = max(1, int(max_bad_row_attempts))
        self._allow_oxts_fallback = allow_oxts_fallback
        # KAIROS_ALLOW_MISSING_LIDAR=false disables the zero-LiDAR fallback,
        # restoring strict failure for any row that lacks a lidar_path.
        if allow_missing_lidar is None:
            allow_missing_lidar = (
                os.environ.get("KAIROS_ALLOW_MISSING_LIDAR", "true").lower() != "false"
            )
        self._allow_missing_lidar = allow_missing_lidar
    ...
    @staticmethod
    def _optional_path(
        row: pd.Series, key: str, fallback_key: Optional[str] = None
    ) -> str:
        """Return the path string if present and valid, else '' (never raises).

        Used for optional sensor paths (lidar_path, lidar_path_t_minus_1,
        oxts_path) where missing data is represented by zero tensors rather than
        a hard failure.  Image paths must still use _path (strict).
        """
        val = row.get(key)
        if not _is_missing_path_value(val):
            return str(val).strip()
        if fallback_key is not None:
            fb = row.get(fallback_key)
            if not _is_missing_path_value(fb):
                return str(fb).strip()
        return ""
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        max_attempts = (
            max(1, min(self._max_bad_row_attempts, len(self)))
            if self._skip_bad_rows else 1
        )
        for attempt in range(max_attempts):
            # Deterministic nearby index: walk forward from idx, wrapping.
            cur_idx = (idx + attempt) % len(self)
            try:
                return self._load_one(cur_idx)
            except Exception as exc:
                if not _is_skippable_data_error(exc):
                    raise  # model/code bugs and non-404 S3 errors never skip
                last_exc = exc
                row = self._df.iloc[cur_idx]
                _warn_bad_row(cur_idx, row, exc, attempt + 1)
                if not self._skip_bad_rows:
                    raise
                _inc_skipped_bad_row(cur_idx, exc)
        raise RuntimeError(
    ...
    def _load_one(self, idx: int) -> dict:
        ...
        # LiDAR paths are optional: ZOD sequences may lack LiDAR entirely.
        # When KAIROS_ALLOW_MISSING_LIDAR=false, revert to strict failure.
        if self._allow_missing_lidar:
            lidar_t_path  = self._optional_path(row, "lidar_path")
            lidar_t1_path = self._optional_path(row, "lidar_path_t_minus_1", "lidar_path")
        else:
            lidar_t_path  = self._path(row, "lidar_path")
            lidar_t1_path = self._path(row, "lidar_path_t_minus_1", "lidar_path")

        _lidar_t_missing  = not lidar_t_path
        _lidar_t1_missing = not lidar_t1_path
        _inc_missing_lidar(idx, _lidar_t_missing, _lidar_t1_missing)

        # Parallel S3 downloads — I/O bound, GIL released during network I/O.
        # 3 images + up to 2 LiDAR frames (replaced by zero-tensor if path absent).
        # Calib submitted separately to reuse the same executor.
        _mpts = self._max_pts

        def _zero_lidar() -> np.ndarray:
            return np.zeros((_mpts, 4), dtype=np.float32)
        ...
        with ThreadPoolExecutor(max_workers=6) as ex:
            ...
            fut_lid_t  = (
                ex.submit(_load_lidar, lidar_t_path,  cache, _mpts)
                if lidar_t_path else ex.submit(_zero_lidar)
            )
            fut_lid_t1 = (
                ex.submit(_load_lidar, lidar_t1_path, cache, _mpts)
                if lidar_t1_path else ex.submit(_zero_lidar)
            )
...
def _check_vram_threshold(tag: str, rank: int = 0) -> None:
    """Fail fast if VRAM exceeds KAIROS_VRAM_FAIL_THRESHOLD_GB (rank-0, GPU only).

    Only active when KAIROS_FAIL_ON_HIGH_VRAM=true.  Raises RuntimeError if
    allocated or max_allocated exceeds the threshold on the rank-0 GPU.
    """
    if rank != 0 or not torch.cuda.is_available():
        return
    if os.environ.get("KAIROS_FAIL_ON_HIGH_VRAM", "").lower() not in ("1", "true", "yes"):
        return
    threshold_gb = float(os.environ.get("KAIROS_VRAM_FAIL_THRESHOLD_GB", "22.0"))
    alloc_gb = torch.cuda.memory_allocated() / 1e9
    max_gb   = torch.cuda.max_memory_allocated() / 1e9
    if alloc_gb > threshold_gb or max_gb > threshold_gb:
        raise RuntimeError(
            f"[vram] FAIL at {tag}: "
            f"allocated={alloc_gb:.2f}GB  max_allocated={max_gb:.2f}GB  "
            f"threshold={threshold_gb:.0f}GB  "
            f"(raise limit via KAIROS_VRAM_FAIL_THRESHOLD_GB, "
            f"or disable via KAIROS_FAIL_ON_HIGH_VRAM=false)"
        )
    print(
        f"[vram] {tag}  allocated={alloc_gb:.2f}GB  "
        f"max_allocated={max_gb:.2f}GB  threshold={threshold_gb:.0f}GB  OK",
        flush=True,
    )
...
def _set_core_trainable(model, trainable: bool) -> None:
    ...
    # Only toggle params that were trainable when the optimizer groups were
    # built (snapshot taken in train() as _core_optim_param_ids).  Without the
    # guard, freeze_core_steps would flip requires_grad=True on tier-frozen
    # core params the optimizer never updates: autograd then allocates grads
    # for ~3.26B params, and under ZeRO-3 those grads hit 0-sized partitioned
    # placeholders with no reduce hooks (shape-0 mismatch crash).
    allowed = getattr(m, "_core_optim_param_ids", None)
    for param in m.hybrid_core.parameters():
...
# ─────────────────────────────────────────────────────────────────────────────
# Trainable tier control — controls which parameters enter the optimizer
# ─────────────────────────────────────────────────────────────────────────────

_TRAINABLE_TIERS = frozenset({
    "smoke_minimal", "adapters_only", "fusion_decoder", "full_budget", "full",
    "g6_48_native_dino_smoke", "g6_48_native_dino_budget",
})


def apply_trainable_tier(model: "KairosModel", tier: str, rank: int) -> None:
    """Set requires_grad on model parameters according to the trainable tier.

    Tier semantics (all set requires_grad BEFORE optimizer groups are built):

    smoke_minimal  (~100-150M trainable)
        Freeze: DINO backbone+LoRA, lidar_encoder, imu_encoder, hybrid_core.
        Train:  vision non-DINO projections (temporal fusion), calib_gate,
                s2ft_decoder, det_head.
        Purpose: full real-data pipeline with tiny optimizer state on g6.12.
        Backward flows through frozen hybrid_core (grad-checkpoint recomputes it)
        to reach calib_gate and temporal fusion — no optimizer state for core.

    adapters_only  (~120-180M trainable)
        Freeze: DINO backbone non-LoRA, lidar_encoder, imu_encoder, hybrid_core.
        Train:  DINO LoRA, vision projections, calib_gate, s2ft_decoder, det_head.

    fusion_decoder  (~500M-1.5B trainable)
        Freeze: DINO backbone (all), hybrid_core large dense weights.
        Train:  lidar_encoder, imu_encoder, calib_gate, s2ft_decoder, det_head,
                hybrid_core norm/router/bias params.
        For ml.g6.48xlarge.

    full_budget  (~1.5B-2B trainable)
        Freeze: DINO backbone non-LoRA only.
        Train:  everything else (DINO LoRA, all encoders, core, decoder).
        g6.48 only.

    full  (no restrictions)
        No requires_grad changes — same as default behavior.
    """
    if tier not in _TRAINABLE_TIERS:
        raise ValueError(
            f"Unknown KAIROS_TRAINABLE_TIER={tier!r}. "
            f"Supported: {sorted(_TRAINABLE_TIERS)}"
        )
    if tier == "full":
        if rank == 0:
            print(f"[trainable] tier=full: no requires_grad changes", flush=True)
        return

    dino = getattr(model.vision_encoder, "dinov2", None)

    if tier == "smoke_minimal":
        # Freeze DINO backbone + LoRA (all of dinov2 sub-module)
        if dino is not None:
        ...
        for p in model.hybrid_core.parameters():
            p.requires_grad_(False)
        # calib_gate, s2ft_decoder, det_head, vision non-DINO projections → trainable

    elif tier == "adapters_only":
        # Freeze DINO backbone non-LoRA (keep LoRA trainable)
        if dino is not None:
        ...
        for p in model.hybrid_core.parameters():
            p.requires_grad_(False)
        # DINO LoRA, vision projections, calib_gate, s2ft_decoder, det_head → trainable

    elif tier == "fusion_decoder":
        # Freeze DINO backbone (all params)
        if dino is not None:
            for p in dino.parameters():
                p.requires_grad_(False)
        # Freeze hybrid_core large dense weights, keep norm/router/bias trainable
        for name, p in model.hybrid_core.named_parameters():
            if not any(k in name for k in ("norm", "router", "expert_bias", "bias")):
                p.requires_grad_(False)
        # lidar, imu, calib_gate, s2ft_decoder, det_head → trainable

    elif tier == "full_budget":
        # Freeze DINO backbone non-LoRA only
        if dino is not None:
            for name, p in dino.named_parameters():
                if "lora_" not in name:
    ...
    elif tier == "g6_48_native_dino_smoke":
        # g6.48 native DINO 518 smoke tier — same freeze pattern as smoke_minimal.
        # DINO+LoRA frozen, LiDAR frozen, IMU frozen, hybrid_core frozen.
        # Trainable: temporal fusion, patch_proj, calib_gate, s2ft_decoder, det_head.
        # Enforces <=300M trainable via KAIROS_MAX_TRAINABLE_PARAMS=300000000.
        if dino is not None:
            for p in dino.parameters():
                p.requires_grad_(False)
        for p in model.lidar_encoder.parameters():
    ...
    elif tier == "g6_48_native_dino_budget":
        # g6.48 native DINO 518 budget tier — only DINO+LoRA frozen.
        # LiDAR, IMU, hybrid_core, calib_gate, decoders trainable.
        # Only use after g6_48_native_dino_smoke passes. Enforces <=600M via
        # KAIROS_MAX_TRAINABLE_PARAMS=600000000.
        if dino is not None:
            for p in dino.parameters():
                p.requires_grad_(False)
        # lidar_encoder, imu_encoder, hybrid_core, calib_gate, decoders → trainable

    if rank == 0:
        _tot, _tr = _param_count(model)
        _fr = _tot - _tr
        print(
            f"[trainable] tier={tier}  trainable={_tr/1e6:.1f}M  "
            f"frozen={_fr/1e6:.1f}M  total={_tot/1e6:.1f}M",
            flush=True,
        )
        for _cname, _cmod in [
            ...
            ("calib_gate",     model.calib_gate),
            ("hybrid_core",    model.hybrid_core),
            ("s2ft_decoder",   model.s2ft_decoder),
            ("det_head",       model.det_head),
        ]:
            _ct, _ctr = _param_count(_cmod)
            print(
                f"[trainable]   {_cname:<14} total={_ct/1e6:.1f}M  "
                f"trainable={_ctr/1e6:.1f}M",
                flush=True,
            )


def _check_trainable_budget(model: "KairosModel", rank: int) -> int:
    """Validate trainable param count against KAIROS_MAX_TRAINABLE_PARAMS budget.

    Returns the trainable param count.
    Raises RuntimeError if over budget unless KAIROS_ALLOW_LARGE_TRAINABLE=true.
    """
    ...
    _est_adam_gb  = (_trainable * 12) / 1e9   # fp32 master + m + v = 12 bytes/param
    _est_total_gb = (_trainable * 16) / 1e9   # + BF16 grad (2) + BF16 param (2)

    _max_raw = os.environ.get("KAIROS_MAX_TRAINABLE_PARAMS", "").strip()
    if not _max_raw:
        if rank == 0:
            print(
                f"[params] trainable_total={_trainable/1e6:.2f}M  "
                f"frozen_total={_frozen/1e6:.2f}M  "
                f"estimated_adamw_state_gb={_est_adam_gb:.2f}  "
                f"budget_limit=none",
                flush=True,
            )
        return _trainable
    ...
    try:
        _max_params = int(_max_raw)
    except ValueError:
        if rank == 0:
            print(
                f"[params] WARN KAIROS_MAX_TRAINABLE_PARAMS={_max_raw!r} is not an int "
    ...
    if rank == 0:
        print(
            f"[params] trainable_total={_trainable/1e6:.2f}M  "
            f"frozen_total={_frozen/1e6:.2f}M  "
            f"budget_limit={_max_params/1e6:.0f}M  "
            f"estimated_adamw_state_gb={_est_adam_gb:.2f}  "
            f"estimated_total_memory_gb={_est_total_gb:.2f}",
            flush=True,
        )

    if _trainable > _max_params:
        _allow_large = os.environ.get(
            "KAIROS_ALLOW_LARGE_TRAINABLE", ""
        ).lower() in ("1", "true", "yes")
        if not _allow_large:
            raise RuntimeError(
                f"[trainable] FATAL: trainable_params={_trainable/1e6:.1f}M exceeds "
                f"KAIROS_MAX_TRAINABLE_PARAMS={_max_params/1e6:.0f}M. "
                f"Estimated GPU AdamW state: {_est_adam_gb:.1f} GB. "
                f"Use KAIROS_TRAINABLE_TIER=smoke_minimal/adapters_only to reduce, "
                f"or set KAIROS_ALLOW_LARGE_TRAINABLE=true to override, "
                f"or run on ml.g6.48xlarge."
            )
        if rank == 0:
            print(
                f"[params] WARN trainable={_trainable/1e6:.1f}M > budget "
                f"({_max_params/1e6:.0f}M) but KAIROS_ALLOW_LARGE_TRAINABLE=true — "
                "proceeding.",
                flush=True,
            )
    return _trainable
...
def _load_latest_checkpoint(
    ...
    # Rank-0 downloads once — single-node training shares /tmp, and concurrent
    # per-rank downloads of the same keys into the same paths corrupt files.
    # The status broadcast keeps all ranks on the same path afterwards: either
    # everyone calls the collective load_checkpoint, or everyone starts fresh.
    download_status: Optional[str] = "ok"
    if rank == 0:
        try:
    ...
    download_status = _broadcast_string(download_status, src=0, device=device)
...
# ─────────────────────────────────────────────────────────────────────────────
# Training reports — local /tmp/kairos_reports + OUTPUT_S3/<job>/reports/
# ─────────────────────────────────────────────────────────────────────────────

def _report_dir() -> Path:
    """Local directory for report artifacts. Always available, even without OUTPUT_S3."""
    REPORTS_LOCAL.mkdir(parents=True, exist_ok=True)
    return REPORTS_LOCAL


def _sm_job_name() -> str:
    """Best-effort SageMaker training job name from SM_TRAINING_ENV, else ''."""
    try:
        env = json.loads(os.environ.get("SM_TRAINING_ENV", "{}"))
        return str(env.get("job_name") or "")
    except Exception:
        return ""


def _append_metrics_jsonl(record: Dict) -> None:
    """Append one JSON line per logged train/val event to training_metrics.jsonl."""
    path = _report_dir() / "training_metrics.jsonl"
    with open(path, "a") as f:
        f.write(json.dumps(record) + "\n")


def _write_training_report(
    args: argparse.Namespace, record: Dict, final: bool = False
) -> Tuple[str, str]:
    """Render a human-readable run summary as (txt, markdown)."""
    ...
    rows = [
        ("Status",               status),
        ("Last update (UTC)",    record.get("timestamp_utc")),
        ("Step",                 f"{record.get('step')} / {args.total_steps}"),
        ("Phase",                record.get("phase", "n/a")),
        ("Dataset S3 (gold_s3)", args.gold_s3 or "n/a"),
        ("Checkpoint S3",        args.ckpt_s3 or "n/a"),
        ("Output S3",            OUTPUT_S3 or "n/a"),
        ("total_loss",           _fmt(record.get("total_loss"))),
        ("s2ft_loss",            _fmt(record.get("s2ft_loss"))),
        ("val_total",            _fmt(record.get("val_total"))),
        ("val_s2ft",             _fmt(record.get("val_s2ft"))),
        ("raw_moe_z_loss",       _fmt(record.get("raw_moe_z_loss"))),
        ("weighted_z_loss",      _fmt(record.get("weighted_z_loss"))),
        ("grad_norm",            _fmt(record.get("grad_norm"))),
        ("learning_rate",        _fmt(record.get("learning_rate"), 3)),
        ("samples_per_sec",      _fmt(record.get("samples_per_sec"), 3)),
        ("vram_gb",              _fmt(record.get("vram_gb"), 3)),
        ("missing_lidar_seen",   record.get("missing_lidar_seen")),
        ("lidar_fallbacks_used", record.get("lidar_fallbacks_used")),
        ("bad_oxts_seen",        record.get("bad_oxts_seen")),
        ("oxts_fallbacks_used",  record.get("oxts_fallbacks_used")),
        ("skipped_bad_rows",     record.get("skipped_bad_rows")),
        ("checkpoint_path",      record.get("checkpoint_path") or "n/a"),
    ]

    txt_lines = ["Kairos-4B Training Report", "=" * 40]
    for k, v in rows:
        txt_lines.append(f"{k:<22}: {v}")
    txt_lines.append("")
    txt = "\n".join(txt_lines)

    md_lines = ["# Kairos-4B Training Report", "", "| Field | Value |", "|---|---|"]
    for k, v in rows:
...
def _maybe_write_pdf(report_text: str, out_path: Path) -> bool:
    """Render `report_text` to `out_path` via reportlab. Returns True on success."""
    try:
        from reportlab.lib.pagesizes import LETTER
        from reportlab.lib.styles import getSampleStyleSheet
        from reportlab.platypus import Preformatted, SimpleDocTemplate
    except ImportError:
        print("[report][warn] reportlab not installed — skipping training_report.pdf")
        return False

    try:
        styles = getSampleStyleSheet()
        doc = SimpleDocTemplate(str(out_path), pagesize=LETTER)
        doc.build([Preformatted(report_text, styles["Code"])])
        return True
    except Exception as e:
        print(f"[report][warn] PDF generation failed: {e}")
        return False


def _upload_report_dir_to_s3(output_s3: str, rank: int) -> Optional[str]:
    """Upload all files in the local report dir to `<output_s3>/<job_name>/reports/`.

    Returns the destination s3:// URI on success, or None if skipped.
    """
    if rank != 0 or not output_s3:
        return None

    bucket, _, prefix = output_s3.removeprefix("s3://").partition("/")
    job_name = _sm_job_name()
    parts = [p for p in (prefix.rstrip("/"), job_name, "reports") if p]
    reports_prefix = "/".join(parts)

    s3 = boto3.client("s3", region_name=AWS_REGION)
    d = _report_dir()
    for fp in sorted(d.iterdir()):
        if fp.is_file():
            s3.upload_file(str(fp), bucket, f"{reports_prefix}/{fp.name}")

    return f"s3://{bucket}/{reports_prefix}/"


def _maybe_write_reports(
    ...
    """Append a metrics record and (re)write/upload the training report bundle.

    Rank-0 only. Never raises — failures are logged as `[report][warn]` so a
    reporting bug can never take down a training run.
    """
    if rank != 0:
        return
    if not _env_flag("KAIROS_ENABLE_TRAINING_REPORTS", True):
        return
    ...
    try:
        record = {
            ...
            "s2ft_loss":            s2ft_val,
            "val_total":            (val_losses or {}).get("val_total"),
            "val_s2ft":             (val_losses or {}).get("val_s2ft"),
            "raw_moe_z_loss":       raw_z_val,
            "weighted_z_loss":      raw_z_val * args.z_loss_coeff,
            "grad_norm":            grad_norm,
            "learning_rate":        lr,
            "samples_per_sec":      samp_per_s,
            "vram_gb":              vram_gb,
            "missing_lidar_seen":   missing_lidar_seen,
            "lidar_fallbacks_used": lidar_fallbacks_used,
            "bad_oxts_seen":        bad_oxts_seen,
            "oxts_fallbacks_used":  oxts_fallbacks_used,
            "skipped_bad_rows":     skipped_bad_rows,
        }
        if checkpoint_path:
            record["checkpoint_path"] = checkpoint_path

        _append_metrics_jsonl(record)

        txt, md = _write_training_report(args, record, final=final)
        d = _report_dir()
        (d / "training_report.txt").write_text(txt)
        (d / "training_report.md").write_text(md)
        _maybe_write_pdf(txt, d / "training_report.pdf")

        uri = _upload_report_dir_to_s3(OUTPUT_S3, rank)
        if uri:
            print(f"[report] uploaded reports -> {uri}")
    except Exception as e:
        print(f"[report][warn] failed to write/upload training reports: {e}")
...
def train(args: argparse.Namespace) -> None:
    ...
    print(
        f"[kairos_train] BUILD_ID=zod-structured-lidar-v24  "
    ...
    print(
        f"[dist] BUILD_ID=zod-structured-lidar-v24  "
        f"rank={rank}  local_rank={local_rank}  world_size={world_size}  "
        f"MASTER_ADDR={os.environ['MASTER_ADDR']}  "
        f"MASTER_PORT={os.environ['MASTER_PORT']}  "
    ...
    if rank == 0:
        print(
            f"[dist_info] BUILD_ID=zod-structured-lidar-v24  "
            f"RANK={rank}  LOCAL_RANK={local_rank}  WORLD_SIZE={world_size}  "
    ...
    # ── Env-gated cap/worker overrides (unset → mode values pass through) ─────
    run_max_pts, run_max_prompt, run_max_target = _apply_env_cap_overrides(
        run_max_pts, run_max_prompt, run_max_target, rank
    )
    # The S2FT decoder positional table is sized by max_gen_len; it must cover
    # the teacher-forcing sequence (max_target - 1 positions) or pos_emb
    # indexing fails when KAIROS_MAX_TARGET raises the target cap.
    if run_max_gen_len < run_max_target:
        if rank == 0:
            print(
                f"[mode] max_gen_len raised {run_max_gen_len}->{run_max_target} "
                "to cover max_target (decoder pos_emb sizing)",
                flush=True,
            )
        run_max_gen_len = run_max_target
    # KAIROS_NUM_WORKERS: overlap S3/decode with GPU compute via DataLoader
    # workers.  NOTE: with workers > 0 the data-quality counters (bad_oxts_seen,
    # missing_lidar_seen, …) live in worker processes and read 0 in main-process
    # logs/reports; per-row [WARN] lines still appear in CloudWatch.
    run_workers = _env_int("KAIROS_NUM_WORKERS", run_workers)

    # KAIROS_SKIP_BAD_ROWS=true enables bad-row skipping even in full training
    # (long runs should not crash on one missing S3 object).  Precedence:
    # explicit --skip_bad_rows > env flag > mode default.
    _env_skip_bad_rows = _env_flag("KAIROS_SKIP_BAD_ROWS", False)
    run_skip_bad_rows = (
        args.skip_bad_rows
        if args.skip_bad_rows is not None
        else (_env_skip_bad_rows
              or bool(args.smoke_mode or args.ultra_smoke_mode or args.budget_mode))
    )
    # Validation stays strict unless KAIROS_SKIP_BAD_ROWS=true is set explicitly.
    run_val_skip_bad_rows = _env_skip_bad_rows
    run_max_bad_row_attempts = _env_int("KAIROS_MAX_BAD_ROW_ATTEMPTS", 100)
    run_preflight_rows = (
        args.preflight_rows
    ...
    if rank == 0:
        ...
        print(
            ...
            f"skip_bad_rows={run_skip_bad_rows}  "
            f"val_skip_bad_rows={run_val_skip_bad_rows}  "
            f"max_bad_row_attempts={run_max_bad_row_attempts}  "
            f"preflight_rows={run_preflight_rows}  "
    ...
    def _dl(df: pd.DataFrame, shuffle: bool = True,
        ...
        return _make_dataloader(
            ...
            skip_bad_rows = (run_skip_bad_rows if skip_bad_rows is None
                             else skip_bad_rows),
    ...
    dl_val,    _           = _dl(df_val, shuffle=False,
                                 skip_bad_rows=run_val_skip_bad_rows)
    ...
    # KAIROS_DINO_SEQUENTIAL_FRAMES=false batches all frames into ONE DINO call —
    # faster when DINO runs under no_grad (frozen) and VRAM headroom exists.
    # Default True preserves the validated smoke behavior.
    if args.smoke_mode or args.budget_mode:
        cfg.vcfg.sequential_frames = _env_flag("KAIROS_DINO_SEQUENTIAL_FRAMES", True)
        if rank == 0:
            print(
                f"[mode] DINO sequential_frames={cfg.vcfg.sequential_frames}  "
                "(env KAIROS_DINO_SEQUENTIAL_FRAMES; False = batched frames, "
                "lower latency / higher peak VRAM)",
                flush=True,
            )
    ...
    # Allow KAIROS_FREEZE_CORE_IN_SMOKE env var to activate core freeze without CLI flag.
    # Emergency fallback: when optimizer alone doesn't clear the OOM, freeze the
    # ~3.26B hybrid core entirely.  Default false — this eliminates gradient flow to the core.
    if os.environ.get("KAIROS_FREEZE_CORE_IN_SMOKE", "").lower() in ("1", "true", "yes"):
        if args.freeze_core_in_smoke is None:
            args.freeze_core_in_smoke = True
    run_freeze_core  = (args.freeze_core_in_smoke
                        if args.freeze_core_in_smoke  is not None
                        else False)

    # ── Trainable tier: controls which parameters enter the optimizer ──────────
    # Applied before per-component freeze so tier takes precedence.
    # After apply_trainable_tier sets requires_grad, the existing freeze section
    # still runs to set no_grad instance flags (important for activation memory).
    _trainable_tier_raw = os.environ.get("KAIROS_TRAINABLE_TIER", "").lower().strip()
    _instance_profile   = os.environ.get("KAIROS_INSTANCE_PROFILE", "auto").lower()
    _inst_type          = getattr(args, "instance_type", "unknown")

    # Auto-detect tier from instance profile when not explicitly set
    if not _trainable_tier_raw:
        if _instance_profile == "g6_12_safe" or (
            _instance_profile == "auto"
            and ("g6.12" in _inst_type or "g5.12" in _inst_type)
            and (args.smoke_mode or args.ultra_smoke_mode)
        ):
            _trainable_tier_raw = "smoke_minimal"
        elif _instance_profile in ("g6_48_balanced", "g6_48_high_detail") or (
            _instance_profile == "auto"
            and "g6.48" in _inst_type
            and not (args.smoke_mode or args.ultra_smoke_mode)
        ):
            _trainable_tier_raw = "fusion_decoder"
        else:
            _trainable_tier_raw = "full"

    if _trainable_tier_raw not in _TRAINABLE_TIERS:
        if rank == 0:
            print(
                f"[trainable] WARN unknown tier={_trainable_tier_raw!r}; "
                f"valid: {sorted(_TRAINABLE_TIERS)}. Defaulting to full.",
                flush=True,
            )
        _trainable_tier_raw = "full"
    ...
    if rank == 0:
        print(
            f"[trainable] tier={_trainable_tier_raw}  "
            f"instance_profile={_instance_profile}  "
            f"instance_type={_inst_type}",
            flush=True,
        )

    # Apply tier (sets requires_grad before optimizer groups are built)
    if _trainable_tier_raw != "full":
        apply_trainable_tier(model, _trainable_tier_raw, rank)
        # Sync existing freeze flags so the existing section's no_grad logic
        # matches the tier (tier is authoritative; flags are updated here)
        if _trainable_tier_raw in ("smoke_minimal", "adapters_only"):
            if not run_freeze_lidar:
                run_freeze_lidar = True
            if not run_freeze_imu:
                run_freeze_imu = True
            if not run_freeze_core:
                run_freeze_core = True
    ...
    # ── Trainable budget guard ────────────────────────────────────────────────
    # Runs after all freeze flags are applied; raises before optimizer creation
    # if trainable params exceed KAIROS_MAX_TRAINABLE_PARAMS (AdamW OOM guard).
    _check_trainable_budget(model, rank)

    # ── Optimizer + WSD schedule ──────────────────────────────────────────────
    _eff_zero_optim = getattr(args, "zero_stage", None)
    if _eff_zero_optim is None:
        _eff_zero_optim = 3 if not (
            getattr(args, "smoke_mode", False) or
            getattr(args, "budget_mode", False) or
            getattr(args, "ultra_smoke_mode", False)
        ) else 2
    _foreach_raw    = os.environ.get("KAIROS_ADAMW_FOREACH", "").lower()
    _optim_safe_raw = os.environ.get("KAIROS_OPTIMIZER_SAFE_MODE", "").lower()
    _optim_safe = (
        _optim_safe_raw in ("1", "true", "yes") or
        (
            (
                getattr(args, "smoke_mode", False) or
                getattr(args, "ultra_smoke_mode", False) or
                getattr(args, "budget_mode", False)
            ) and
            _eff_zero_optim == 3 and
            _optim_safe_raw not in ("0", "false", "no")
        )
    )
    _use_foreach = (
        _foreach_raw in ("1", "true", "yes") and not _optim_safe
    )
    _adamw_extra = _build_adamw_kwargs(_use_foreach)

    # Offload env vars still read for DS config (offload_optimizer section).
    # They do NOT select the Python optimizer — use KAIROS_FORCE_DEEPSPEED_CPUADAM
    # to explicitly request DeepSpeedCPUAdam (requires prebuilt ops).
    _offload_opt = (
        os.environ.get("KAIROS_ZERO_OFFLOAD_OPTIMIZER", "").lower() in ("1", "true", "yes") or
        os.environ.get("KAIROS_ZERO_OFFLOAD", "").lower() in ("1", "true", "yes") or
        getattr(args, "zero_offload", False)
    )
    _offload_params = (
        os.environ.get("KAIROS_ZERO_OFFLOAD_PARAMS", "").lower() in ("1", "true", "yes") or
        getattr(args, "zero_offload", False)
    )
    _force_cpu_adam       = os.environ.get("KAIROS_FORCE_DEEPSPEED_CPUADAM", "").lower() in ("1", "true", "yes")
    _smoke_optim_override = os.environ.get("KAIROS_SMOKE_OPTIMIZER", "").lower()

    # Snapshot which hybrid-core params are trainable NOW (post-tier/freeze).
    # _set_core_trainable (freeze_core_steps warmup) must never widen
    # requires_grad beyond this set — see _set_core_trainable for why.
    model._core_optim_param_ids = frozenset(
        id(p) for p in model.hybrid_core.parameters() if p.requires_grad
    )

    _param_groups = model.param_groups(
    ...
    if rank == 0:
        _opt_total    = sum(p.numel() for g in _param_groups for p in g["params"])
        _frozen_total = sum(p.numel() for p in model.parameters() if not p.requires_grad)
        for g in _param_groups:
            _grp_n = len(g["params"])
            _grp_m = sum(p.numel() for p in g["params"])
            print(
                f"[params] trainable_{g['name']}={_grp_m/1e6:.2f}M"
                f"  (param_tensors={_grp_n})",
                flush=True,
            )
        print(f"[params] optimizer_trainable_param_count={_opt_total/1e6:.2f}M", flush=True)
        print(f"[params] frozen_excluded_count={_frozen_total/1e6:.2f}M", flush=True)
        print(f"[optim] zero_offload_optimizer={_offload_opt}", flush=True)
        print(f"[optim] offload_optimizer_device={'cpu' if _offload_opt else 'none'}", flush=True)
        print(f"[optim] offload_param_device={'cpu' if _offload_params else 'none'}", flush=True)
        print(f"[optim] force_cpu_adam={_force_cpu_adam}", flush=True)

    # Select optimizer class.
    #   KAIROS_FORCE_DEEPSPEED_CPUADAM=true: use CPUAdam (explicit opt-in only).
    #     If unavailable (ops not compiled, libcurand missing) → fail fast.
    #   KAIROS_SMOKE_OPTIMIZER=sgd: correctness-only SGD fallback in smoke mode.
    #   Default: GPU AdamW (foreach=False, fused controlled by KAIROS_ADAMW_FUSED).
    #
    # NOTE: CPUAdam is NOT the default fix for g6.12xlarge OOM.  The primary fix is
    # KAIROS_TRAINABLE_TIER=smoke_minimal which reduces trainable params to ~100M
    # (optimizer state ~1.2 GB vs ~41 GB for 3.44B params).  CPUAdam is available
    # as an opt-in for custom images where the DeepSpeed CPU Adam extension is prebuilt.
    _cpu_adam_used = False

    if _force_cpu_adam:
        if not _HAS_DS_CPU_ADAM:
            raise RuntimeError(
                "[optim] FATAL: KAIROS_FORCE_DEEPSPEED_CPUADAM=true but "
                "DeepSpeedCPUAdam is not available (DeepSpeed CPU ops not compiled; "
                "the SageMaker DLC may be missing libcurand). "
                "Unset KAIROS_FORCE_DEEPSPEED_CPUADAM or use a custom image with "
                "the deepspeed CPU Adam extension prebuilt."
            )
        optimizer = _DeepSpeedCPUAdam(
            _param_groups,
            weight_decay          = args.weight_decay,
            adamw_mode            = True,
            fp32_optimizer_states = True,
        )
        _optim_class_name = "DeepSpeedCPUAdam"
        _cpu_adam_used    = True
        if rank == 0:
            print(
                "[optim] DeepSpeedCPUAdam selected via KAIROS_FORCE_DEEPSPEED_CPUADAM=true",
                flush=True,
            )
    elif (
        _smoke_optim_override == "sgd" and
        (getattr(args, "smoke_mode", False) or getattr(args, "ultra_smoke_mode", False))
    ):
        # SGD has no Adam fp32 state → near-zero GPU optimizer memory.
        # Correctness-smoke only — NOT suitable for real training.
        optimizer = torch.optim.SGD(
            _param_groups,
            momentum     = 0.0,
            weight_decay = args.weight_decay,
        )
        _optim_class_name = "torch.optim.SGD"
        if rank == 0:
            print(
                "[optim] WARN using SGD smoke fallback (KAIROS_SMOKE_OPTIMIZER=sgd). "
                "NOT intended for final training — correctness-smoke only.",
                flush=True,
            )
    else:
        if _offload_opt and not _force_cpu_adam and rank == 0:
            print(
                "[optim] NOTE: KAIROS_ZERO_OFFLOAD_OPTIMIZER=true affects the DS config "
                "but the Python optimizer is GPU AdamW (set KAIROS_FORCE_DEEPSPEED_CPUADAM=true "
                "to use DeepSpeedCPUAdam for the actual parameter update on CPU).",
                flush=True,
            )
        optimizer = AdamW(
            _param_groups,
        ...
        _optim_class_name = "torch.optim.AdamW"

    if rank == 0:
        print(
            f"[optim] optimizer_class={_optim_class_name}  "
            f"device={'cpu' if _cpu_adam_used else 'gpu'}  "
            f"cpu_adam_used={_cpu_adam_used}  "
            f"AdamW foreach={_use_foreach}  "
            f"fused={_adamw_extra.get('fused', 'n/a') if not _cpu_adam_used else 'n/a (CPUAdam)'}  "
            f"optim_safe_mode={_optim_safe}",
            flush=True,
        )

    _lr_mode, _min_lr_ratio = _pick_lr_schedule(args)
    if _lr_mode in ("constant", "smoke"):
        warmup_steps = 0
        stable_steps = args.total_steps
    else:
        warmup_steps = int(args.total_steps * args.warmup_frac)
        stable_steps = int(args.total_steps * args.stable_frac)
    _decay_steps = max(0, args.total_steps - warmup_steps - stable_steps)
    _lr_fn       = _build_lr_lambda(warmup_steps, stable_steps, args.total_steps,
                                     _lr_mode, _min_lr_ratio)
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=_lr_fn)
    if rank == 0:
        _base_lrs = [f"{g['lr']:.2e}" for g in optimizer.param_groups]
        print(f"[lr] schedule={_lr_mode}  base_lrs={_base_lrs}", flush=True)
        print(
            f"[lr] warmup_steps={warmup_steps}  stable_steps={stable_steps}  "
            f"decay_steps={_decay_steps}",
            flush=True,
        )
        print(f"[lr] min_lr_ratio={_min_lr_ratio:.2f}", flush=True)
        if _lr_mode in ("constant", "smoke"):
            print("[lr] NOTE: constant schedule — LR fixed at 1× base (no decay)", flush=True)
        elif _min_lr_ratio > 0.0:
            print(
                f"[lr] NOTE: LR floored at {_min_lr_ratio:.0%} of base "
                f"— use KAIROS_ALLOW_ZERO_LR=true to remove floor",
                flush=True,
            )
    ...
    # Allow KAIROS_ZERO3_DEBUG_SAFE container env var to activate conservative settings
    # even when --zero3_debug_safe was not passed as a CLI hyperparameter.
    if os.environ.get("KAIROS_ZERO3_DEBUG_SAFE", "").lower() in ("1", "true", "yes"):
        if not getattr(args, 'zero3_debug_safe', False):
            args.zero3_debug_safe = True
            if rank == 0:
                print("[zero3] KAIROS_ZERO3_DEBUG_SAFE=true → zero3_debug_safe activated "
                      "(overlap_comm=False, contiguous_gradients=True, small buckets)",
                      flush=True)
    ...
    if rank == 0:
        ...
        # CUDA allocator and ZeRO-3 config summary
        _alloc_conf = os.environ.get("PYTORCH_CUDA_ALLOC_CONF", "not set")
        _expandable = "expandable_segments:True" in _alloc_conf
        print(f"[cuda_alloc] PYTORCH_CUDA_ALLOC_CONF={_alloc_conf}", flush=True)
        print(f"[cuda_alloc] expandable_segments_enabled={_expandable}", flush=True)
        _z_opt = _ds_cfg_dict.get("zero_optimization", {})
        if _z_opt.get("stage") == 3:
            print(f"[zero3] overlap_comm={_z_opt.get('overlap_comm', 'N/A')}", flush=True)
            print(f"[zero3] contiguous_gradients={_z_opt.get('contiguous_gradients', 'N/A')}", flush=True)
            print(f"[zero3] reduce_bucket_size={_z_opt.get('reduce_bucket_size', 'N/A')}", flush=True)
            print(f"[zero3] stage3_prefetch_bucket_size={_z_opt.get('stage3_prefetch_bucket_size', 'N/A')}", flush=True)
            print(f"[zero3] offload_optimizer={_z_opt.get('offload_optimizer', 'none')}", flush=True)
            print(f"[zero3] offload_param={_z_opt.get('offload_param', 'none')}", flush=True)
    ...
    _allow_zero_lr = os.environ.get("KAIROS_ALLOW_ZERO_LR", "").lower() in ("1","true","yes")

    # Latest known scalars — kept up to date for report hooks (validation,
    # checkpoint, final) that may fire on steps without a log_every print.
    total_val = s2ft_val = z_val = 0.0
    grad_norm = float("nan")
    last_samp_per_s = 0.0
    last_val_losses: Optional[Dict[str, float]] = None
    ...
    while step < args.total_steps:
        ...
        loss = None   # release computation graph immediately after backward;
                      # output still holds loss scalar refs for post-step logging
        # Optional: empty CUDA allocator cache before optimizer step to release
        # fragmented cached blocks and reduce peak allocation during engine.step().
        _do_empty_cache = os.environ.get(
            "KAIROS_EMPTY_CACHE_BEFORE_OPTIM_STEP", ""
        ).lower() in ("1", "true", "yes")
        if not _diag_done:
            if _do_empty_cache and torch.cuda.is_available():
                _log_cuda_mem("before_empty_cache", rank)
                torch.cuda.empty_cache()
                _log_cuda_mem("after_empty_cache", rank)
            _log_cuda_mem("before_optimizer_step", rank)
        elif _do_empty_cache and torch.cuda.is_available():
            torch.cuda.empty_cache()
        engine.step()           # advances micro_steps; calls scheduler.step()
                                # automatically at gradient_accumulation boundary
        if not _diag_done:
            _log_cuda_mem("after_optimizer_step", rank)
            _check_vram_threshold("after_optimizer_step", rank)
            _diag_done = True
        micro_step += 1
        ...
        if micro_step % args.grad_accum == 0:
            ...
            if step % args.log_every == 0 and rank == 0:
                ...
                print(
                    ...
                    f"missing_lidar_seen={missing_lidar_seen} "
                    f"lidar_fallbacks_used={lidar_fallbacks_used} "
                    f"skipped_bad_rows={skipped_bad_rows}",
                    flush=True,
                )
                _cur_lrs = _get_all_lrs(optimizer)
                print(
                    f"[lr] step={step} group_lrs={[f'{lr:.2e}' for lr in _cur_lrs]}",
                    flush=True,
                )
                # Zero-LR fail-fast: nonzero LR required for meaningful training.
                # Skip: smoke runs (total_steps < 50), near final step (>95%), explicit opt-out.
                if (args.total_steps >= 50
                        and step < int(args.total_steps * 0.95)
                        and not _allow_zero_lr
                        and all(lr == 0.0 for lr in _cur_lrs)):
                    raise RuntimeError(
                        f"[lr] FAIL: all LR groups are zero at step {step}/"
                        f"{args.total_steps} (before final 5%). "
                        "WSD schedule may have decayed too fast for this total_steps. "
                        "Fix: use --lr_schedule budget (auto-applied for <=500 steps), "
                        "set KAIROS_LR_SCHEDULE=budget, or "
                        "set KAIROS_ALLOW_ZERO_LR=true to suppress this check."
                    )
                _check_vram_threshold(f"step_{step}", rank)
                acc_total = acc_s2ft = acc_z = acc_vutil = acc_text_ratio = 0.0
                acc_count = 0

            # ── Validation ────────────────────────────────────────────────────
            if run_val_every > 0 and step % run_val_every == 0:
                # KAIROS_VAL_MAX_BATCHES: batches per rank per validation pass
                # (default 50 ≈ 400 samples at mb=1 × 8 ranks — raise for a
                # lower-variance val estimate at the cost of val wall time).
                val_losses = _validate(
                    engine, dl_val, device, rank,
                    max_batches=_env_int("KAIROS_VAL_MAX_BATCHES", 50),
                )
                ...
                if rank == 0 and _env_flag("KAIROS_REPORT_EVERY_VAL", True):
                    _maybe_write_reports(
                        args, rank, step, cur_phase,
                        total_val=total_val, s2ft_val=s2ft_val, raw_z_val=z_val,
                        grad_norm=grad_norm, lr=_get_lr(optimizer),
                        samp_per_s=last_samp_per_s, vram_gb=_gpu_mem_gb(),
                        val_losses=val_losses,
                    )

            # ── Checkpoint ────────────────────────────────────────────────────
            if run_ckpt_enabled:
                # Rank-0 decides when a checkpoint is due and broadcasts the
                # decision.  last_ckpt_t drifts across ranks — rank-0 alone
                # spends time uploading to S3 after each save — so a per-rank
                # time check would let ranks enter the collective
                # engine.save_checkpoint at different steps (NCCL mismatch → hang).
                _ckpt_due = (
                    rank == 0
                ...
                if dist.is_initialized():
                    _due_t = torch.tensor(
                        1.0 if _ckpt_due else 0.0, device=device
                    )
                    dist.broadcast(_due_t, src=0)
                    _ckpt_due = bool(_due_t.item())
                if _ckpt_due:
                    ...
                    if rank == 0:
                        _maybe_write_reports(
                            ...
                            checkpoint_path=(
                                f"{args.ckpt_s3.rstrip('/')}/{_ckpt_tag(step)}/"
                                if args.ckpt_s3 else None
                            ),
                        )
    ...
    if rank == 0:
        _maybe_write_reports(
            ...
            checkpoint_path=(
                f"{args.ckpt_s3.rstrip('/')}/{_ckpt_tag(step)}/"
                if (run_ckpt_enabled and args.ckpt_s3) else None
            ),
            final=True,
        )

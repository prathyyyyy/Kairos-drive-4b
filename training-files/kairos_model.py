...
class KairosModel(nn.Module):
    ...
    def forward(self, batch: KairoBatch) -> KairosOutput:
        ...
        if batch.target_bytes is not None and _skip_dec_loss:
            ...
            # Teacher forcing.
            # Legacy default: input = target[:-1], label = target[1:].  This never
            # feeds BOS at position 0, but generate() conditions on BOS — a
            # train/inference mismatch (first generated byte is out-of-distribution).
            # KAIROS_DECODER_PREPEND_BOS=true: input = [BOS] + target[:-1],
            # label = target — aligns training with generate().  Default OFF so the
            # validated smoke-run loss path is preserved bit-for-bit.
            _prepend_bos = _os.environ.get(
                "KAIROS_DECODER_PREPEND_BOS", ""
            ).strip().lower() in ("1", "true", "yes")
            if _prepend_bos:
                bos_col = batch.target_bytes.new_full(
                    (batch.target_bytes.shape[0], 1), cfg.bos_id
                )
                dec_input  = torch.cat(
                    [bos_col, batch.target_bytes[:, :-1].clamp(min=0)], dim=1
                )                                                # (B, T_tgt)
                dec_label  = batch.target_bytes.clone()          # (B, T_tgt)
                ...
                label_mask = (batch.loss_mask[:, 1:]
                              if batch.loss_mask is not None else None)
            if label_mask is not None:
                dec_label = dec_label.masked_fill(~label_mask.bool(), -1)
            logits    = self.s2ft_decoder(x, dec_input)      # (B, T_dec, vocab)
            ...
            if label_mask is not None:
                mask = label_mask.reshape(-1).float()
